...
public class DictionaryController {
    ...
    public List<Entry> getWordsEndingWith(@PathVariable String value) {
        ...
        StopWatch sw = new StopWatch();
        sw.start();
        List<Entry> entries = dictionaryService.getWordsEndingWith(value);
        sw.stop();
    ...
    public List<Entry> getWordsThatContainConsecutiveLetters() {
        ...
        long nanoSeconds = sw.getLastTaskTimeNanos();
        String message = new StringBuilder().append("Retrieved entries for words containing consecutive double letters ")
                .append(" containing ")
                .append(entries.size())
                .append(" entries in ")
                .append(nanoSeconds / 1000000.0)
